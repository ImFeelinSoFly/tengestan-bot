<?php	
	...
	$id_zakaz = intval($_POST['us_id_zakaz']);
	$amount = $_POST['AMOUNT'];
	$sign = $_POST['SIGN'];
	
	$merchant_id = get_setting_data('pub_key',9);
	$secret_word = get_setting_data('secret_key',9);
	$sign_recv = md5($merchant_id.':'.$amount.':'.$secret_word.':'.$id_zakaz);
	if($sign !== $sign_recv){ echo 'error sign';exit; }
	...
	if($type_payment == 1)
	{
		$r = mysqli_query($db,"SELECT `id_chat`,`amount`,`bot_id` FROM `payment_users` 
						WHERE `pay_type` = '9' AND `status` = '0' AND `id` = '$id_zakaz' AND `amount` = '$amount'");
		...
		mysqli_query($db,"UPDATE `payment_users` SET `status` = '2', `amount` = '$amount' WHERE `id` = '$id_zakaz'");
	...
	}else
	{
		$r = mysqli_query($db,"SELECT `id`,`id_chat`,`suma`,`pay_type`,`bot_id`,`id_wallet_qiwi`,`discount` FROM `a_zakaz` 
														WHERE `status` = '1' AND `id` = '$id_zakaz' AND `pay_type` = '9'");
		$exist = mysqli_num_rows($r);
		if($exist < 0){ exit; }
		$data_x = mysqli_fetch_assoc($r);
		$task_id = $data_x['id'];
